...
def main():
    ...
    # String replacement necessary to properly match symbols from rna.fna
    ...
    mouse_dna_seq_dict: dict[str, list[str]] = defaultdict(list)
    mouse_protein_seq_dict: dict[str, list[str]] = defaultdict(list)
    ...
    mouse_seq_dict = fetch_sequences_from_local_ncbi(
        mouse_fasta_records, mouse_dna_seq_dict, symbols
    )
    ...
    mouse_seq_dict = fetch_sequences_from_local_strings(
        strings_df, strings_fasta_records, mouse_protein_seq_dict, symbols
    )

    ## I tried querying ncbi for the remaining sequences but ncbi took an incredibly long time to fetch sequences
    # remaining_recs = symbols - set(mouse_seq_dict.keys()) # There's now a protein and dna dict
    # mouse_seq_dict = fetch_sequences_from_ncbi(list(remaining_recs), mouse_seq_dict)

    recs_with_seqs = set(mouse_dna_seq_dict.keys()) | set(mouse_protein_seq_dict)
    ...
    # A few hundred thousand rows were removed from the df 1,881,859 -> 1,368,611
    print(filtered_df.describe())
    """
           Regulator Symbol Target Symbol Regulator Type
    count           1368611       1368611        1368611
    unique            10086         20064              2
    top            Mirlet7a        Elavl1          miRNA
    freq              16194          2369        1013291
    """
    ...
    sequence_info_file = "../dna_seq_info.json"
    with open(sequence_info_file, "w") as file:
        json.dump(mouse_dna_seq_dict, file, indent=4)
    sequence_info_file = "../protein_seq_info.json"
    with open(sequence_info_file, "w") as file:
        json.dump(mouse_protein_seq_dict, file, indent=4)
